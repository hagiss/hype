...
class HypClassifer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_norm = torch.norm(x, dim=-1, keepdim=True) * 0.1
        weight_norm = torch.norm(self.weight, dim=-1, keepdim=True).T.squeeze() * 0.1

        logits = x @ self.weight.T + self.bias
        c=0.1
        return (logits.detach() * (x_norm * weight_norm) + logits * (x_norm * weight_norm).detach()) / 2
